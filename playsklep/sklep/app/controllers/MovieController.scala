...
class MovieController @Inject()(
  ...
  def get(movieId: String) : Action[AnyContent] = Action.async { implicit request: Request[_]  =>
    ...
    movieRepository.getById(movieId) map {
      case Some(m) => Ok(views.html.movie.movie(m, comments, ratings, actors, directors, filmtypes))
  ...
  def create: Action[AnyContent] = Action.async{ implicit request: Request[_]  =>
    ...
    Ok(views.html.movie.add_movie(createMovieForm, actors, directors, filmtypes))
  }

  def delete(movieId: String) : Action[AnyContent] = Action.async { implicit request: Request[_]  =>
    movieRepository.delete(movieId).map(_ => Redirect(routes.MovieController.getAll()).flashing("info" -> "Film usunięty"))
  }

  def createMovieHandler: Action[AnyContent] = Action.async { implicit request: Request[_]  =>
    ...
    val successFunction = { movie: CreateMovieForm =>
      movieRepository.create(movie.title, movie.details, movie.publicationDate, movie.price, movie.actors, movie.directors, movie.filmtypes).map { _ =>
  ...
  def update(movieId: String): Action[AnyContent] = Action.async { implicit request: Request[_]  =>
    ...
    val filmtypes: Seq[Filmtype] = Await.result(filmtypeRepository.getAll(), Duration.Inf)
    val selectedFilmtypes: Seq[String] = Await.result(filmtypeRepository.getForMovie(movieId), Duration.Inf).map(_.id)

    val updateForm = createMovieForm.fill(
      CreateMovieForm(movie.title, movie.details, movie.publicationDate, movie.price, selectedActors, selectedDirectors, selectedFilmtypes)
    )
  ...
  def updateMovieHandler(movieId: String): Action[AnyContent] = Action.async { implicit request: Request[_]  =>
    val errorFunction = { formWithErrors: Form[CreateMovieForm] =>
      Future {
    ...
    val successFunction = { movie: CreateMovieForm =>
      movieRepository.update(movieId, movie.title, movie.details, movie.publicationDate, movie.price, movie.actors, movie.directors, movie.filmtypes).map { _ =>
